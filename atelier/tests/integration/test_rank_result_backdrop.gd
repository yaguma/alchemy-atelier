...
const SCENE_PATH := "res://features/rank/ui/rank_result_backdrop.tscn"
const RankResultBackdropPixelScript = preload("res://features/rank/ui/rank_result_backdrop.gd")


func _make_backdrop() -> RankResultBackdropPixel:
	var runner := scene_runner(SCENE_PATH)
	return runner.scene() as RankResultBackdropPixel
...
func test_texture_がBACKDROP_TEXTUREと一致する() -> void:
	...
	assert_object(backdrop.texture).is_equal(RankResultBackdropPixelScript.BACKDROP_TEXTURE)
...
func test_ノード名がRankResultBackdropである() -> void:
	var backdrop := _make_backdrop()

	assert_str(backdrop.name).is_equal("RankResultBackdrop")
