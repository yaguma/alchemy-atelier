...
func test_setupで名称と品質と特性と貢献度と報酬が表示に反映される() -> void:
	...
	assert_str(_find_label(row, "NameLabel").text).is_equal("回復薬")
	assert_str(_find_label(row, "QualityLabel").text).contains("4")
	assert_str(_find_label(row, "TraitsLabel").text).contains("holy")
	var value_text := _find_label(row, "ValueLabel").text
...
func test_order_matchedがtrueのとき指定合致ラベルが表示される() -> void:
	...
	var order_label := _find_label(row, "OrderMatchLabel")
...
func test_order_matchedがfalseのとき指定合致ラベルが非表示になる() -> void:
	...
	assert_bool(_find_label(row, "OrderMatchLabel").visible).is_false()
...
func test_複数の発現特性がすべて表示される() -> void:
	...
	var traits_text := _find_label(row, "TraitsLabel").text
...
func test_setupを再実行すると前回の表示内容が残らない() -> void:
	...
	assert_str(_find_label(row, "NameLabel").text).is_equal("鉄塊")
	assert_str(_find_label(row, "TraitsLabel").text).not_contains("holy")
	assert_str(_find_label(row, "ValueLabel").text).not_contains("12.5")
	assert_bool(_find_label(row, "OrderMatchLabel").visible).is_false()
...
func test_調合物名が空文字でもクラッシュせず表示される() -> void:
	...
	assert_str(_find_label(row, "NameLabel").text).is_empty()
	assert_str(_find_label(row, "QualityLabel").text).contains("1")
...
func test_発現特性が空配列のとき特性なし表記になる() -> void:
	...
	assert_str(_find_label(row, "TraitsLabel").text).contains(
		GuildDeliveryResultRow.TRAITS_NONE_TEXT
	)
...
func test_品質スコアの下限と上限が表示できる() -> void:
	...
	assert_str(_find_label(row, "QualityLabel").text).contains(str(GameBalance.QUALITY_SCORE_MIN))

	row.setup("回復薬", GameBalance.QUALITY_SCORE_MAX, no_traits, false, 0.0, 0.0)
	assert_str(_find_label(row, "QualityLabel").text).contains(str(GameBalance.QUALITY_SCORE_MAX))
...
func test_貢献度と報酬が0でも表示される() -> void:
	...
	assert_str(_find_label(row, "ValueLabel").text).contains("0.0")
