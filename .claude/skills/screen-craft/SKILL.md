...
### 2. 必要な画像アセットを揃える（atelier-image-genを使う）

レイヤー分解の結果、背景装飾（`TextureRect`）・素材/調合物アイコン・UI装飾パーツのように
「画像そのもの」が要る要素が出てきたら、まず`atelier/assets/`配下に流用できる既存アセットが
ないか確認する。無ければ自分で仮のプレースホルダーを用意したり画像生成を後回しにしたりせず、
その場で`atelier-image-gen`スキルを呼び出して生成する（Skillツールで`skill: atelier-image-gen`を
指定し、種別・用途・置き場所を`args`で伝える）。このプロジェクトは`atelier/assets/`配下がほぼ
未着手（フォント以外ほぼ空）なので、演出を作り込んでも肝心の画像がなければ「見て気持ちいいUI」
まで到達しない。

- 単色背景・`UiTheme`の色を敷くだけの`ColorRect`で十分な場合は画像生成は不要（`layers.md`
  「背景レイヤー」参照）。イラスト・アイコンとして絵柄が要る場合のみ生成する
- 生成した画像は`atelier-image-gen`のレビュー手順（生成後のReadでの目視確認）に従ってから
  画面に組み込む。トーンが既存アイコンと合わない場合は、そのアイコンを`--reference-image`に
  渡して再生成できることも伝える
- 1画面で複数種の画像（背景1枚＋アイコン数点等）が要る場合も、依頼された分だけ生成する
  （`atelier-image-gen`の「生成コストとレート制限」の注意点を踏襲し、バリエーション違いの
  大量生成はしない）

### 3. 要素ごとにアニメーション手法を割り当てる
