...
> 🔴 2026-08-06改訂: 技術スタックがGodot 4.x + GDScriptに確定済み（`CLAUDE.md`参照）のため、「Warning の基準」内のTypeScript固有項目をGDScript向けに調整した。それ以外の基準（重要度定義・行動規範）は言語非依存のため変更なし。
...
- メモリリーク（Autoloadへのsignal購読解除忘れ、リソース未解放）

### Warning の基準

以下のいずれかに該当する場合:
- アーキテクチャルールへの違反（Feature-Based, Functional Core分離）
- 型安全性の欠如（`Variant`型の無条件使用, 不適切なキャスト, 型注釈の欠落）
...
| **並列性** | なし | 5並列 |
| **議論分析** | なし | クロスカッティング分析 |
| **出力先** | docs/code-reviews/ | PRコメント（Issue/PR上に残す） |
