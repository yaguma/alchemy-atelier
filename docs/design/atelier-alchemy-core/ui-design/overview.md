...
要件定義書・コンセプト文書のスコープに基づき、対象画面を以下9画面とする（ユーザーとの確認により確定）。セーブロード画面（スロット選択画面）は2026-09-05時点で実装済み（PR #45）のため対象に含める（🔵2026-09-05修正。旧版は「タイトル画面・設定画面・セーブロード画面は…対象外とする」としていたが、セーブ/ロード機能がスコープに追加され実装されたため修正した）。タイトル画面・設定画面・一時停止メニューは要件定義書の設計スコープ外のままだったが、2026-09-04付でPR #46として実装され（マージ先が`main`ではなく`feat/save-load-system`ブランチだったため長らく`main`に未反映）、2026-09-06に`main`へ統合の上、一時停止メニュー等を拡張した（🔵2026-09-06修正、詳細は`docs/dev/plans/title-settings-screens-extension/`参照）。
...
stateDiagram-v2
    [*] --> SCR_008_Title
    SCR_008_Title --> SCR_007_SlotSelect: はじめから／つづきから（どちらも同じ遷移先）
    SCR_008_Title --> SCR_009_Settings: せってい
    SCR_007_SlotSelect --> SCR_001_Garden: スロット選択（新規/復元とも同一画面へ進む）
    SCR_001_Garden --> SCR_009_Settings: HUDのメニューボタン（一時停止メニュー経由）
    SCR_009_Settings --> SCR_008_Title: タイトルに戻る（一時停止メニューのみ、確認ダイアログなし）
...
🔵 **2026-09-06追加（実装〔PR #46・title-settings-screens-extensionプラン〕反映）**: `BootScene`の直後の表示先が`SCR-007`から`SCR-008`（タイトル画面）へ変更された。`SCR-008`の「はじめから」「つづきから」はいずれも同じ`SCR-007`へ遷移し、新規/継続の実際の判定は従来どおり`SCR-007`側の責務のまま変わらない（「終了」ボタンは`get_tree().quit()`を呼ぶのみで独立した遷移先を持たないため遷移図には表現していない）。ゲーム中の`SCR-009`（一時停止メニュー）は確認ダイアログなしで即座に`SCR-008`へ戻る（既存のオートセーブが進行を保護するため）。新規ゲーム開始時に前回プレイの`GameState`を引き継いでしまう不具合は`GameState.reset_for_new_game()`で修正済み（`SaveService.select_slot_and_restore()`の新規スロット分岐から呼ばれる）。
